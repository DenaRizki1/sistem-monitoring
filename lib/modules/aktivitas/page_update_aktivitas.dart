...
class PageUpdateAktivitas extends StatefulWidget {
  final Map dataAktivitas;

  const PageUpdateAktivitas({Key? key, required this.dataAktivitas}) : super(key: key);

  @override
  State<PageUpdateAktivitas> createState() => _PageUpdateAktivitasState();
}

class _PageUpdateAktivitasState extends State<PageUpdateAktivitas> {
  ...
  @override
  void initState() {
    _dataAktivitas = widget.dataAktivitas;
    WidgetsBinding.instance.addPostFrameCallback((_) {
      setState(() {
        _tglAktivitasDate = DateFormat("yyyy-MM-dd").parse(_dataAktivitas['tgl_aktivitas'].toString());
        _tglAktivitasC.text = parseDateInd(_dataAktivitas['tgl_aktivitas'].toString(), "dd MMMM yyyy");
        _aktivitasC.text = _dataAktivitas['aktivitas'].toString();
        List listFoto = jsonDecode(_dataAktivitas['foto_aktivitas'].toString());
        _listFotoAktivitas = listFoto;
      });
    });
  ...
  Future<Map<String, dynamic>?> updateAktivitas() async {
    await showLoading();
    ...
    Map<String, String> params = {
      ...
      'hash_aktivitas': _dataAktivitas['hash_aktivitas'].toString(),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: appBarWidget("Update Aktivitas"),
      body: Stack(
        children: [
          SizedBox(
              ...
              ElevatedButton(
                onPressed: () async {
                  if (_tglAktivitasDate == null) {
                    ...
                    if (result ?? false) {
                      final response = await updateAktivitas();
                      if (response != null) {
                        if (response['success']) {
